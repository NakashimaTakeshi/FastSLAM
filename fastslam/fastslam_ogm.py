...
class FastSlam(MCL):
    ...
    def _fastslam_measurement_model(self, particle, observations, landmarks, i, scalefactor=1.0):
        ...
        for observation in observations:
            ...
            k = list(filter(lambda feature: feature._landmark_id == j, particle._map))
            if len(k) == 0:
                print("newlandmark!!!!!!!!!!")
                ...
                particle._map.append(FastSlamFeature(mu, Sigma, j))  # self.particle_set[i]._features.append(FastSlamFeature(mu, Sigma, j))  # TODO: weight の更新が必要かどうか確認する
            else:
                print("oldlandmark!!!!!!!!!!")
                l = ([i for i, feature in enumerate(particle._map) if feature._landmark_id == j])[0]
                feature = k[0]
                ...
                particle._map[l]._mean = feature._mean + np.ravel(K * np.matrix(z - z_hat).T).T
                particle._map[l]._covariance_matrix = (np.eye(2) - K * H) * feature._covariance_matrix
    ...
    def _fastslam_resampling(self, particle_set: list):
        ...
        for i in range(self._n_particle):
            ...
            new_particle_set.append(copy.deepcopy(FastSlamParticle(id=i, x=particle_set[survival_index].pose.x, y=particle_set[survival_index].pose.y, theta=particle_set[
                survival_index].pose.theta, weight=1.0 / self._n_particle, features=particle_set[survival_index]._map)))

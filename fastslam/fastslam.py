...
class FastSlam(MCL):
    ...
    def __init__(self, n_particle: int = 30, x_region: int = 200, y_region: int = 200):
        ...
        self.privious_observed_landmarks = None
        # super().__init__(n_particle, x_region, y_region)
    ...
    def update_particles(self, odometry, observations,landmarks):
        ...
        if n_eff < 0.5 * self._n_particle or sum(nomalized_weights) < 1.0e-15:
            print("resampling")
            self.particle_set = self._fastslam_resampling(self.particle_set)
            # print([particle._weight for particle in self.particle_set])
    ...
    def _fastslam_measurement_model(self, particle, observations,landmarks,i):
        ...
        for observation in observations:
            ...
            if len(k) == 0:
                ...
                particle._features.append(FastSlamFeature(mu, Sigma, j))
                # self.particle_set[i]._features.append(FastSlamFeature(mu, Sigma, j))
                #TODO: weight の更新が必要かどうか確認する
            else:
                print("oldlandmark!!!!!!!!!!")
    ...
    def _calculate_jacobian(pose, mean):
        ...
        H = np.matrix([[-dx / math.sqrt(q), -dy / math.sqrt(q), 0.0],
                    [dy / q, -dx / q, -1.0],
                    [0.0, 0.0, 0.0]])
        return H[:2,:2]
    ...
    #     return np.array([observation.distance, observation.angle,observation.landmark_id])

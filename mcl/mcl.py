...
class MCL:
    ...
    def _sample_motion_model_odometory(previouse_pose, odometry):
        ...
        # update pose
        pose = Pose(previouse_pose._x + delta_translation * math.cos(previouse_pose._theta + delta_rotation1),
                    previouse_pose._y + delta_translation * math.sin(previouse_pose._theta + delta_rotation1),
                    previouse_pose._theta + delta_rotation1 + delta_rotation2)
    ...
    def _mesurement_model( particle, observations,landmarks):
        ...
        # ToDo correspondence　は必ず一致するので、全てのパーティクルに同じ影響を与える。変更が必要か要検討。
        ...
        for observation in observations:
            ...
            likelihood = stats.norm.pdf(observation.distance, distance, std_distance) * \
                        stats.norm.pdf(observation.angle, angle, std_angle) * \
                        stats.norm.pdf(observation.landmark_id, j, std_correspondence)
    ...
    def _resampling(self, particle_set:list):
        ...
        for i in range(self._n_particle):
            rvs = rv.rvs(size=1)
            new_particle_set.append(Particle(id=1, x=particle_set[np.argmax(rvs[0])].pose._x, y=particle_set[np.argmax(rvs[0])].pose._y, theta=particle_set[np.argmax(rvs[0])].pose._theta, weight=1.0 / self._n_particle))
